...
FeatureDetector::FeatureDetector(int w, int h, GlobalSettings& globalSettings_) :
globalSettings(globalSettings_), gW0(w), gH0(h)
{
    HALF_PATCH_SIZE = 19;
    PATCH_SIZE = 31;
    umax = InitUmax();
    ...
    cv::Mat(h, w, CV_8U, cv::Scalar(0)).copyTo(ImageBlurred);
    ThreadPool = shared_ptr<IndexThreadReduce<Vec10>>(new IndexThreadReduce<Vec10>());
    minIndDistPt = cv::Point2f(globalSettings.minIndDist, globalSettings.minIndDist);
    minDirDistPt = cv::Point2i(globalSettings.minDirDist, globalSettings.minDirDist);
}
...
void FeatureDetector::ExtractFeatures(cv::Mat &Image, cv::Mat &Occupancy, std::vector<cv::KeyPoint> &mvKeys, cv::Mat &Descriptors, int &NumFeatures, int NumFeaturesToExtract)
{
    ...
    cv::FAST(ImageBlurred, FASTselection, globalSettings.minThFAST, true);
    ...
    for (int i = 0, iend = FASTselection.size(); i < iend; ++i )
        {
            if(mvKeys.size() >= NumFeaturesToExtract)
                break;
            if(FASTselection[i].pt.y < (HALF_PATCH_SIZE + 1) || (FASTselection[i].pt.y > (gH0 - HALF_PATCH_SIZE - 1)))
                continue;
            if(FASTselection[i].pt.x < (HALF_PATCH_SIZE + 1) || (FASTselection[i].pt.x > (gW0 - HALF_PATCH_SIZE - 1)))
                continue;
            
            int index = FASTselection[i].pt.y * gW0 + FASTselection[i].pt.x;
